...
const BlogInteraction = () => {
    ...
    useEffect(() => {
        if(access_token){
            //send req, to get if liked by user
                    ...
                    <div className="flex gap-3 items-center ">
                        <button onClick ={handleLike}
                         className={"w-10 h-10 rounded-full flex items-center justify-center " + ( isLikedByUser ? "bg-red/20 text-red " : "bg-grey/80 ")}>
                            <i className={"fi " + ( isLikedByUser ? "fi-sr-heart " : "fi-rr-heart ") + "leading-none"}></i>
                        </button>
                        <p className="text-xl text-dark-grey">{total_likes}</p>
                    
                        <button
                         onClick={() => setCommentsWrapper(preVal => !preVal)}
                         className="w-10 h-10 rounded-full flex  justify-center items-center bg-grey/80">
                            <i className="fi fi-rr-comment-dots  leading-none "></i>
                        </button>
